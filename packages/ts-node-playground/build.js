...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .sv-type { font-size: 11px; font-weight: 600; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.4px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
...
function renderScriptView() {
  ...
  nodes.forEach(n => {
    maxX = Math.max(maxX, (n.x ?? 0) + (n.width ?? 4) + 1);
    maxY = Math.max(maxY, (n.y ?? 0) + (n.height ?? 2) + 1);
  });
  ...
  canvas.innerHTML = nodes.map(node => {
    ...
    const desc = isEntry && node.description
      ...
      : '';
    return \`<div class="sv-node\${isEntry ? ' sv-entry' : ''}" style="left:\${x}rem;top:\${y}rem;width:\${w}rem;height:\${h}rem">
      <div class="sv-type">\${esc(label)}</div>
      \${desc}
    </div>\`;
  }).join('');
}
