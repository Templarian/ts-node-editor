...
async function renderScriptView() {
  ...
  canvas.innerHTML = nodes.map(node => {
    ...
    if (isEntry) {
      const headerPin = node.args?.nodes != null ? \`<span class="sv-header-pin" data-arg-key="nodes" onmousedown="startConnDrag(event,0,'nodes')"></span>\` : '';
    ...
    const headerPin = node.args?.nodes != null ? \`<span class="sv-header-pin" data-arg-key="nodes" onmousedown="startConnDrag(event,\${node.id},'nodes')"></span>\` : '';
